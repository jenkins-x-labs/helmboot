...
NAME := helmboot
BUILD_TARGET = build
MAIN_SRC_FILE=cmd/helmboot/helmboot.go
...
GO_DEPENDENCIES := $(call rwildcard,pkg/,*.go) $(call rwildcard,cmd/helmboot/,*.go)
...
build: $(GO_DEPENDENCIES) ## Build helmboot binary for current OS
...
test: make-reports-dir ## Run tests with the "unit" build tag
	KUBECONFIG=/cluster/connections/not/allowed CGO_ENABLED=$(CGO_ENABLED) $(GOTEST) -failfast ./... $(TEST_BUILDFLAGS)
...
testbin: get-test-deps make-reports-dir
	CGO_ENABLED=$(CGO_ENABLED) $(GOTEST) -c github.com/jenkins-x/helmboot/pkg/cmd -o build/helmboot-test $(TEST_BUILDFLAGS)
...
release: clean build test-slow-integration linux # Release the binary
	...
	@if [[ -z "${DISTRO}" ]]; then \
		./build/linux/helmboot step changelog --verbose --header-file=docs/dev/changelog-header.md --version=$(VERSION) --rev=$(PULL_BASE_SHA) --output-markdown=changelog.md --update-release=false; \
